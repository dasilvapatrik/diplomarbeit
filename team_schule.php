    <section id="team" class="bg-light-gray">
        <div class="container">
            <div class="row">
                <div class="col-lg-12 text-center">
					<img class="center-block img-responsive" src="img/logos/logo_ohnetext300.png" alt="">
                    <h2 class="section-heading">Wir sind Buchwiesen</h2>
                    <!--<h3 class="section-subheading text-muted">Lorem ipsum dolor sit amet consectetur.</h3>-->
                </div>
            </div>

            <div class="row">
                <div class="col-lg-12 text-center">
					<p class="large text-muted"><h2>WER SIND WIR</h2></p>
						<p class="large text-muted">Unsere Schule gehört zu den über 110 öffentlichen Schulen der Stadt Zürich und bietet neben
						dem regulären Unterricht im Kindergarten und der Primarschule auch zahlreiche weitere <strong>attraktive Angebote</strong> für die Schülerinnen und Schüler.</p>
						
						<p class="large text-muted">Die Schuleinheit Buchwiesen umfasst 4 Kindergärten, 8 Unterstufenklassen und 6 Mittelstufenklassen. Zusätzlich zum regulären Unterricht 
						bieten wir diverse zusätzliche Angebote im Bereich Schule und Betreuung an.</p>
						
						<p class="large text-muted">Insgesamt besuchen über 380 Kinder die Schule Buchwiesen. Die Kinder werden von rund 30 Klassen- und ca. 25 Fachlehrpersonen unterrichtet.
						Im Betreuungsbereich sind rund 20 Hortleitende und Assistenzen im Einsatz.</p>
					
					
					<p class="large text-muted"><h2>WAS ZEICHNET UNS AUS</h2></p>
						<p class="large text-muted">Wir sehen uns als eine Einheit aus Unterrichtsteam und Betreuungsteam. Gemeinsam verfolgen wir das Ziel den Schülerinnen und Schülern 
						der Schule Buchwiesen eine gute, ansprechende und unterstützende Förderung anzubieten. Eine gute Zusammenarbeit zwischen allen Beteiligten in unserem Schulbetrieb
						ist uns wichtig. Sie fördert das Lernen der Kinder, erhöht die Akzeptanz der Schule in der Öffentlichkeit und entlastet die Einzelperson. Um dies gewährleisten zu
						können arbeiten die Betreuung und die Unterrichtsteams Hand in Hand. </p>
					
					
					<p class="large text-muted"><h2>WIE ARBEITEN WIR</h2></p>
						<p class="large text-muted">Die gute Zusammenarbeit wird aufgebaut durch Wertschätzung, gegenseitige Information und - falls nötig
						- die Koordination von pädagogischen Massnahmen.</p>

							<ul class="large text-muted text-left">
							  <li>Sie trägt wesentlich zur Schulzufriedenheit der Eltern und damit zur Schulzufriedenheit der Kinder und Jugendlichen bei.</li>
							  <li>Sie reduziert die Störneigung der Kinder und Jugendlichen im Unterricht und in der Betreuung.</li>
							  <li>Sie führt zu geringerer Belastung derEinzelperson.</li>
							  <li>Sie hilft den Eltern, ihre Kinder zielgerichteter im Schulalltag zu unterstützen.</li>
							  <li>Sie ermöglicht den Eltern, eine realistischere Einstellung bezüglich Schulleistung oder Verhalten ihres Kindes zu bekommen.</li>
							  <li>Sie hilft den Elternkontakt zu strukturieren und allfällige pädagogische Massnahmen zielgerichtet durchzuführen.</li>
							</ul>
				</div>
            </div>
        </div>
		<div class="topabstand text-center">	
			<button type="button" class="btn btn-xl" onClick="window.location.href='https://www.stadt-zuerich.ch/content/ssd/de/index/volksschule/schulkreise_kreisschulpflegen/glattal/schulen/buchwiesen_schuleinheit/schulhaus.html'">Städtische Homepage</button>
		</div>
    </section>